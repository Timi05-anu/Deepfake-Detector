...
print(f"\nLoading training data at {IMAGE_SIZE}...")
start = time.time()
...
callbacks = [
    ModelCheckpoint(filepath=os.path.join(MODEL_DIR, 'best_model_128.h5'),
                     monitor='val_auc', mode='max', save_best_only=True, verbose=1),
    EarlyStopping(monitor='val_auc', mode='max', patience=3,
                  restore_best_weights=True, verbose=1),
    CSVLogger(os.path.join(MODEL_DIR, 'training_log_128.csv'))
]
...
model.fit(X_train, y_train, validation_data=(X_val, y_val),
          epochs=5, batch_size=BATCH_SIZE, callbacks=callbacks)
...
print(f"Average inference time at 128x128: {avg_ms:.1f} ms/image")

model.save(os.path.join(MODEL_DIR, 'final_model_128.h5'))
print(f"SUMMARY: accuracy={results[1]:.4f} auc={results[2]:.4f} inference_ms={avg_ms:.1f}")
